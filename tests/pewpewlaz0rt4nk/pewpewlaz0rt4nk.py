...
class Beam(object):
    ...
    def validate(self):
        ...
        response = []
        received_lines = self._data.splitlines()

        for index, expected in enumerate(self.response):
            received = received_lines[index] if index < len(received_lines) else ''
    ...
    @property
    def data(self):
        """Return the complete decoded response after receive()."""
        return self._data
...
class Laz0rCannon(object):
    ...
    def pewpew(self, settings=True):
        ...
        return self._result['fail'] == 0

    @property
    def result(self):
        """Return a copy of the current pass/fail counters."""
